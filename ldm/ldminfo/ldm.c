...
#else
#define ldm_debug(f, a...) _ldm_printf (KERN_DEBUG, __FUNCTION__, f, ##a)
#endif

#define ldm_crit(f, a...)  _ldm_printf (KERN_CRIT,  __FUNCTION__, f, ##a)
#define ldm_error(f, a...) _ldm_printf (KERN_ERR,   __FUNCTION__, f, ##a)
#define ldm_info(f, a...)  _ldm_printf (KERN_INFO,  __FUNCTION__, f, ##a)

__attribute__ ((format (printf, 3, 4)))
static void _ldm_printf (const char *level, const char *function,
			 const char *fmt, ...)
{
	static char buf[128];
	va_list args;
 ...
 * read_dev_sector
 */
static unsigned char *read_dev_sector(int dev, u64 sector)
{
	int size = 512;		// XXX sector size
	u8 *buffer;
	long long offset;
	...
	buffer = malloc (size);
	if (buffer) {
		if (lseek64 (dev, offset, SEEK_SET) < 0)
			printf (KERN_CRIT "lseek to %lld failed\n", offset);
		else if (read (dev, buffer, size) < size)
			printf (KERN_CRIT "read failed\n");
		else
			goto done;
 ...
 * put_partition
 */
static void put_partition(struct parsed_partitions *p, int n, int from, int size)
{
	if (n < p->limit) {
		p->parts[n].from = from;
		p->parts[n].size = size;
...
static BOOL ldm_parse_tocblock (const u8 *data, struct tocblock *toc)
{
	...
	strncpy ((char *) toc->bitmap1_name, (const char *) data + 0x24, sizeof (toc->bitmap1_name));
	toc->bitmap1_name[sizeof (toc->bitmap1_name) - 1] = 0;
	toc->bitmap1_start = BE64 (data + 0x2E);
	toc->bitmap1_size  = BE64 (data + 0x36);

	if (strncmp ((char *) toc->bitmap1_name, TOC_BITMAP1, sizeof (toc->bitmap1_name)) != 0) {
		ldm_crit ("TOCBLOCK's first bitmap is '%s', should be '%s'.",
				TOC_BITMAP1, toc->bitmap1_name);
		return FALSE;
	}
	strncpy ((char *) toc->bitmap2_name, (const char *) data + 0x46, sizeof (toc->bitmap2_name));
	toc->bitmap2_name[sizeof (toc->bitmap2_name) - 1] = 0;
	toc->bitmap2_start = BE64 (data + 0x50);
	toc->bitmap2_size  = BE64 (data + 0x58);
	if (strncmp ((char *) toc->bitmap2_name, TOC_BITMAP2, sizeof (toc->bitmap2_name)) != 0) {
				   ...
				   const struct tocblock *toc2)
{
	...
	return ((toc1->bitmap1_start == toc2->bitmap1_start)	&&
		...
		!strncmp ((const char *) toc1->bitmap1_name, (const char *) toc2->bitmap1_name, sizeof (toc1->bitmap1_name)) &&
		!strncmp ((const char *) toc1->bitmap2_name, (const char *) toc2->bitmap2_name, sizeof (toc1->bitmap2_name)));
}
 ...
 *          FALSE  Error
 */
static BOOL ldm_validate_privheads (int dev, u64 num_sects, struct privhead *ph1)
{
	...
	ph[1] = malloc (sizeof (*ph[1]));
	ph[2] = malloc (sizeof (*ph[2]));
	if (!ph[1] || !ph[2]) {
	...
	for (i = 0; i < 3; i++) {
		data = read_dev_sector (dev, ph[0]->config_start + off[i]);
		if (!data) {
 ...
 */
static BOOL ldm_validate_tocblocks (int dev, unsigned long base, struct ldmdb *ldb)
{
	...
	ph    = &ldb->ph;
	tb[0] = &ldb->toc;
	tb[1] = malloc (sizeof (*tb[1]));
	tb[2] = malloc (sizeof (*tb[2]));
	tb[3] = malloc (sizeof (*tb[3]));
	if (!tb[1] || !tb[2] || !tb[3]) {
		ldm_crit ("Out of memory.");
		goto out;
	}
	...
	{
		data = read_dev_sector (dev, base + off[i]);
		if (!data) {
			ldm_crit ("Disk read failed.");
			goto out;
		}
 ...
 */
static BOOL ldm_validate_vmdb (int dev, unsigned long base, struct ldmdb *ldb)
{
	u8 *data;
	BOOL result = FALSE;
	struct vmdb *vm;
	...
	data = read_dev_sector (dev, base + OFF_VMDB);
	if (!data) {
		ldm_crit ("Disk read failed.");
		return FALSE;
	}
 ...
 */
static BOOL ldm_validate_partition_table (int dev)
{
	u8 *data;
	struct partition *p;
	int i;
	...
	for (i = 0; i < 4; i++, p++)
		if (SYS_IND (p) == WIN_DYNAMIC_PARTITION) {
			result = TRUE;
			break;
		}
...
static void ldm_frag_free (struct list_head *list)
{
	...
	list_for_each_safe (item, tmp, list)
		free (list_entry (item, struct frag, list));
}
 ...
 *          FALSE  An error occurred
 */
static BOOL ldm_get_vblks (int dev, unsigned long base, struct ldmdb *ldb)
{
	...
	for (s = skip; s < finish; s++) {		/* For each sector */
		data = read_dev_sector (dev, base + OFF_VMDB + s);
		if (!data) {
			ldm_crit ("Disk read failed.");
			goto out;
		}
		...
		ptr = data;
		for (v = 0; v < perbuf; v++, ptr+=size) {  /* For each vblk */
			if (MAGIC_VBLK != BE32 (ptr)) {
				ldm_error ("Expected to find a VBLK.");
				goto out;
			}

			recs = BE16 (ptr + 0x0E);	/* Number of records */
			if (recs == 1) {
				if (!ldm_ldmdb_add (ptr, size, ldb))
					goto out;	/* Already logged */
			} else if (recs > 1) {
				if (!ldm_frag_add (ptr, size, &frags))
					goto out;	/* Already logged */
			}
...
void ldm_free_vblks (struct list_head *lh)
{
	...
	list_for_each_safe (item, tmp, lh) {
		v = list_entry (item, struct vblk, list);
		list_del (item);
		free (v);
	}
}
 ...
 */
int ldm_partition (int dev, struct parsed_partitions *pp, struct ldmdb *ldb)
{
	unsigned long base;
	int result = -1;
	...
	if (!ldm_validate_privheads (dev, pp->parts[0].size, &ldb->ph))
		goto out;		/* Already logged */
	...
	if (!ldm_validate_tocblocks (dev, base, ldb) ||
	    !ldm_validate_vmdb      (dev, base, ldb))
	    	goto out;		/* Already logged */
